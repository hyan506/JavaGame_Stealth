...
public class MultiplayerController implements Initializable{
	...
	private Bird Larry = new Bird();
	public int timeCountDown = 123;
	...
	@FXML public void GamePressed(KeyEvent e)
	{
		...
		if(key == "UP") {
			Larry.setVspeed(-200);
			Larry.setImage("bird_up.png");
		}
		else if(key == "DOWN") {
			Larry.setVspeed(200);
			Larry.setImage("bird_down.png");
		}
		else if(key == "RIGHT") {
			Larry.setHspeed(200);
			Larry.setImage("bird_right.png");
		}
		else if(key == "LEFT") {
			Larry.setHspeed(-200);
			Larry.setImage("bird_left.png");
		}
		...
		makeVGuards:
			...
			makeKeys:
				while (Keys.size()<=3)
				{
					...
					Keys.add(K);
					System.out.println("Key : " + Integer.toString(K.getNumber()));
				}
			//Generate Coins
			...
			makeDoors:
				while (Doors.size()<=1)
				{
					...
					Doors.add(D);
					System.out.println("Door : " + Integer.toString(D.getDoorNumber()));
				}
			//Generate power 
				...
				public void handle(long currentNanoTime)
				{
					...
					if(!paused) {
						...
						for(int i = 0; i<Coins.size(); i++) {
							...
							else if(Larry.Touch(Coin)) {
								Coins.remove(i);
								user.setScore(user.getScore()+Coin.getPoints()-5);
							}
